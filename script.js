...
function showTrustee(index,shouldScroll=false){
  ...
  detail.innerHTML=`<figure class="trustee-detail-portrait"><img src="assets/images/${person.image}" width="900" height="1200" alt="${person.name}"></figure><div class="trustee-detail-copy"><h3>${person.name}</h3><p class="trustee-role">${person.role}</p><p>${person.profile}</p>${person.quote?`<blockquote>“${person.quote}”</blockquote>`:""}</div><div class="trustee-controls"><button type="button" data-direction="-1" aria-label="Previous trustee">←</button><button type="button" data-direction="1" aria-label="Next trustee">→</button></div>`;
